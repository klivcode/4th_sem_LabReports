...
int main() {
    ...
    int *positions = (int *)malloc((numRequests + 2) * sizeof(int)); // +2 for end points
    if (!positions) {
        printf("Memory allocation failed\n");
        free(requests);
        return 1;
    ...
    // Divide requests based on current head
    ...
    if (direction == 1) { // outward (toward higher cylinder)
        // Service right side
        ...
        // Go to end
        positions[totalPoints++] = endRequest;
        totalSeekTime += abs(endRequest - prevPos);
        prevPos = endRequest;
        // Reverse and service left side in descending order
        for (i = leftCount - 1; i >= 0; i--) {
            positions[totalPoints++] = left[i];
            totalSeekTime += abs(left[i] - prevPos);
            prevPos = left[i];
        }
    } else { // inward (toward cylinder 0)
        ...
        // Go to start
        positions[totalPoints++] = 0;
        totalSeekTime += abs(0 - prevPos);
        prevPos = 0;
        // Service right side
        for (i = 0; i < rightCount; i++) {
            positions[totalPoints++] = right[i];
